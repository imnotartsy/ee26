set_property IOSTANDARD LVCMOS33 [get_ports {B[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {B[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {B[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {B[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {S[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {S[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {S[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {S[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports CI]
set_property IOSTANDARD LVCMOS33 [get_ports CO]
set_property IOSTANDARD LVCMOS33 [get_ports T]
set_property IOSTANDARD LVCMOS33 [get_ports V]
set_property IOSTANDARD LVCMOS33 [get_ports Z]
set_property PACKAGE_PIN P4 [get_ports {A[3]}]
set_property PACKAGE_PIN P3 [get_ports {A[2]}]
set_property PACKAGE_PIN R3 [get_ports {A[1]}]
set_property PACKAGE_PIN T1 [get_ports {A[0]}]
set_property PACKAGE_PIN R6 [get_ports {B[3]}]
set_property PACKAGE_PIN R7 [get_ports {B[2]}]
set_property PACKAGE_PIN U8 [get_ports {B[1]}]
set_property PACKAGE_PIN U9 [get_ports {B[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {A[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {A[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {A[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {A[0]}]
set_property PACKAGE_PIN P2 [get_ports CO]
set_property PACKAGE_PIN R2 [get_ports {S[3]}]
set_property PACKAGE_PIN U1 [get_ports {S[2]}]
set_property PACKAGE_PIN P5 [get_ports {S[1]}]
set_property PACKAGE_PIN R1 [get_ports {S[0]}]
set_property PACKAGE_PIN V5 [get_ports T]
set_property PACKAGE_PIN V6 [get_ports CI]
set_property PACKAGE_PIN V9 [get_ports V]
set_property PACKAGE_PIN T8 [get_ports Z]